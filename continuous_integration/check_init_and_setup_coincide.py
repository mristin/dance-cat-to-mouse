...
"""Check that the distribution and dancecattomouse/__init__.py are in sync."""
...
def main() -> int:
    ...
    if setup_py_map["version"] != dancecattomouse.__version__:
        print(
            f"The version in the setup.py is {setup_py_map['version']}, "
            f"while the version in dancecattomouse/__init__.py is: "
            f"{dancecattomouse.__version__}",
            file=sys.stderr,
        )
        success = False

    if setup_py_map["author"] != dancecattomouse.__author__:
        print(
            f"The author in the setup.py is {setup_py_map['author']}, "
            f"while the author in dancecattomouse/__init__.py is: "
            f"{dancecattomouse.__author__}",
            file=sys.stderr,
        )
        success = False

    if setup_py_map["license"] != dancecattomouse.__license__:
        print(
            f"The license in the setup.py is {setup_py_map['license']}, "
            f"while the license in dancecattomouse/__init__.py is: "
            f"{dancecattomouse.__license__}",
            file=sys.stderr,
        )
        success = False

    if setup_py_map["description"] != dancecattomouse.__doc__:
        print(
            f"The description in the setup.py is {setup_py_map['description']}, "
            f"while the description in dancecattomouse/__init__.py is: "
            f"{dancecattomouse.__doc__}",
            file=sys.stderr,
        )
        success = False
    ...
    if status_classifier is None:
        ...
        if expected_status_in_init != dancecattomouse.__status__:
            print(
                f"Expected status {expected_status_in_init} "
                f"according to setup.py in dancecattomouse/__init__.py, "
                f"but found: {dancecattomouse.__status__}"
            )
            success = False
